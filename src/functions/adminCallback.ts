...
export const wishTable = async (ctx:CallbackQueryContext<BotContext>) => {
	...
	let wish = await Database.getRepository(WishTable).find({where:{eventName }})
	let wish_formatted = eventName + ' Wishes\n\n' + wish.map(w => 
		[`Name: ${w.name}`,`Wish: ${w.wish}`].join("\n")
		).join('\n\n')

	// query.select(`wishTable WHERE eventName = "${eventName}"`,"name, wish" );
	const inlineKeyboard = new InlineKeyboard([[{"text": "Back", "callback_data": "wishManagement"}]])
